...
/**
 * StdoutTransport is a transport for Winston that logs to the console in a predefined format with contrasting colors.
 * It can be used in the MAIAR runtime to display logs in the console in a predefined format with contrasting colors.
 * Format: $MAIAR | timestamp | level | type | message | metadata
 */
export const stdout = new transports.Console({
  format: combine(
    ...
    printf(({ level, message, timestamp, type, ...metadata }) => {
      const meta = metadata
      ...
      const log = [
        ...
        level,
        color(type as string, brightGreen),
